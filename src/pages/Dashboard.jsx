...
export default function Dashboard() {
  ...
  const stopRef     = useRef(false);  // signals polling loop to stop
  ...
  // ── LIVE MIC — HTTP polling (replaces WebSocket, works reliably on HuggingFace)
  // Client captures audio → accumulates 2s windows → POST /analyze-chunk → updates UI
  const startRecording = async () => {
    stopAll(); resetState();
    stopRef.current = false;
    setStatus('ANALYZING');

    toast.loading('Waking backend…', { id: 'wake' });
    await warmBackend();
    toast.dismiss('wake');
    ...
    try {
      ...
      const WINDOW_SAMPLES = 32000;  // 2s @ 16kHz
      const STEP_NATIVE    = NATIVE_SR; // 1s of native-rate audio per send
      ...
      // Sliding window buffer (2s @ 16kHz = 32000 samples)
      let windowBuf = new Float32Array(0);
      let counter   = 0;
      // Client-side EMA for smoothing (matches hf_space_app alpha=0.45)
      let ema = null;

      // Sends one window to /analyze-chunk
      const sendChunk = async (window16k) => {
        if (stopRef.current) return;
        try {
          const res = await fetch(`${BACKEND_URL}/analyze-chunk`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/octet-stream' },
            body: window16k.buffer,
            signal: AbortSignal.timeout(10000),
          });
          if (!res.ok) return;
          const d = await res.json();
          if (stopRef.current) return;
          if (d.silence) return; // VAD: skip silent windows

          // EMA smoothing
          const raw = d.score || 0;
          ema = ema === null ? raw : 0.45 * raw + 0.55 * ema;

          // Weighted rolling aggregation (same as backend RiskAggregator)
          counter++;
          const risk = ema > 0.70 ? 'HIGH RISK' : ema > 0.45 ? 'MEDIUM RISK' : 'REAL';
          setRiskScore(ema);
          setRiskLevel(risk);
          setChunks(prev => [...prev, { id: counter, score: ema, risk }]);
          setChartData(prev => [...prev, { t: counter, score: +(ema * 100).toFixed(1) }]);
        } catch { /* network error — just skip this window */ }
      };

      // Audio capture: accumulate native-rate audio → downsample → build sliding window
      const captureAudio = (inputChunk) => {
        const ds = downsample(inputChunk, NATIVE_SR, TARGET_SR);
        const merged = new Float32Array(windowBuf.length + ds.length);
        merged.set(windowBuf); merged.set(ds, windowBuf.length);
        windowBuf = merged;
        // Every 1s of 16kHz audio received → slide window by 1s and send 2s
        while (windowBuf.length >= WINDOW_SAMPLES) {
          const win = windowBuf.slice(0, WINDOW_SAMPLES);
          windowBuf = windowBuf.slice(TARGET_SR); // slide 1s
          sendChunk(win); // fire-and-forget (non-blocking)
        }
      };

      // Prefer AudioWorkletNode, fallback to ScriptProcessor
      try {
        ...
        worklet.port.onmessage = (e) => captureAudio(new Float32Array(e.data));
        src.connect(worklet);
        worklet.connect(ac.destination);
      } catch {
        const proc = ac.createScriptProcessor(4096, 1, 1);
        processorRef.current = proc;
        proc.onaudioprocess = (ev) => captureAudio(ev.inputBuffer.getChannelData(0).slice());
        src.connect(proc);
      ...
      toast.success('Live mic streaming ✓ — HTTP mode (HF compatible)');
